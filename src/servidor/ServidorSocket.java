...
public class ServidorSocket implements Runnable {
	...
	private Map<Integer, AccionDeJuego> _accionesDeJuego;
	...
	public ServidorSocket(final Socket socket) {
		try {
			...
			if (MainServidor.PARAM_MOSTRAR_IP_CONECTANDOSE || MainServidor.MODO_DEBUG) {
				System.out.println("SE ESTA CONECTANDO LA IP " + _IP);
			}
			if (Mundo.BLOQUEANDO) {
			...
			if (MainServidor.PARAM_SISTEMA_IP_ESPERA && !ServidorServer.borrarIPEspera(_IP)) {
...
// disasociar mimobionte
	...
	private synchronized void objeto_Usar(final String packet) {
		try {
			...
			if (_perso.getPelea() != null && (_perso.getPelea().getFase() != Constantes.PELEA_FASE_POSICION || !comestible)) {
				GestorSalida.ENVIAR_Im_INFORMACION(_perso, "191");
			} else if (!Condiciones.validaCondiciones(_perso, objModelo.getCondiciones())) {
				GestorSalida.ENVIAR_Im_INFORMACION(_perso, "119|43");
			} else {
				...
				case Constantes.INTERCAMBIO_TIPO_BOUTIQUE :// boutique
					try {
						...
						if (objModelo.getItemPago() != null) {
							...
							final int ogrinas = objModelo.getOgrinas() * cantidad;
							if (ogrinas < 0) {
								GestorSalida.ENVIAR_EBE_ERROR_DE_COMPRA(_perso);
								return;
							}
