...
public class GestorSQL {
	...
	public static String conexionAlterna(String host, String database, String user, String pass) {
		try {
			if (_bdAlterna != null) {
				try {
					_bdAlterna.close();
	...
	private static boolean SET_OGRINAS_CUENTA(int monto, final int cuentaID) {
		if (MainServidor.PARAM_NO_USAR_OGRINAS) {
			return false;
		}
		if (monto < 0) {
	...
	public static boolean SET_CREDITOS_CUENTA(int monto, final int cuentaID) {
		if (MainServidor.PARAM_NO_USAR_CREDITOS) {
			return false;
		}
		if (monto < 0) {
		...
		String consultaSQL = "UPDATE `cuentas` SET `creditos` = '" + monto + "' WHERE `id` = '" + cuentaID + "'";
		try {
			final PreparedStatement declaracion = transaccionSQL(consultaSQL, _bdCuentas);
			ejecutarTransaccion(declaracion);
			cerrarDeclaracion(declaracion);
	...
	public static void ADD_OGRINAS_CUENTA(final int ogrinas, final int cuentaID) {
		if (MainServidor.PARAM_NO_USAR_OGRINAS) {
			return;
		}
		int exOgrinas = GET_OGRINAS_CUENTA(cuentaID);
		SET_OGRINAS_CUENTA((ogrinas + exOgrinas), cuentaID);
	}
	
	public static void ADD_CREDITOS_CUENTA(final int creditos, final int cuentaID) {
		if (MainServidor.PARAM_NO_USAR_CREDITOS) {
			return;
		}
		int exCreditos = GET_CREDITOS_CUENTA(cuentaID);
		SET_CREDITOS_CUENTA(creditos + exCreditos, cuentaID);
	}
	
	public static boolean RESTAR_OGRINAS(final Cuenta cuenta, final int restar, Personaje perso) {
		if (MainServidor.PARAM_NO_USAR_OGRINAS) {
			return true;
		}
		...
		try {
			...
			if (!SET_OGRINAS_CUENTA(ogrinas - restar, cuenta.getID())) {
				return false;
			}
			resultado = true;
	...
	public static boolean RESTAR_CREDITOS(final Cuenta cuenta, final int restar, Personaje perso) {
		if (MainServidor.PARAM_NO_USAR_CREDITOS) {
			return true;
		}
		boolean resultado = false;
		try {
			...
			if (!SET_CREDITOS_CUENTA(creditos - restar, cuenta.getID())) {
				return false;
			}
			resultado = true;
